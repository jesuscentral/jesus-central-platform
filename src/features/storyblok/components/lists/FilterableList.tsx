        ...
        <div className="mb-10 flex flex-wrap justify-center gap-3">
          {filters.map((f) => (
            <button
              ...
              className={cn(
                'cursor-pointer rounded-full border border-black/10 px-4 py-2 text-sm font-semibold tracking-wide uppercase transition',
                filter === f
            ...
            {filtered.map((event) => (
              <Link href={linkResolver(event.slug as string)} key={event._uid}>
                <motion.article
                  ...
                  className="group relative overflow-hidden rounded-2xl border border-black/5 bg-white shadow-sm transition hover:shadow-lg"
                >
                  {/* Thumbnail */}
                  <div className="relative h-44 w-full overflow-hidden">
                    ...
                    {event.type && (
                      <div className="absolute top-3 left-3 rounded-full bg-[var(--strategy-green)] px-3 py-1 text-xs font-semibold text-[var(--freedom)] uppercase shadow-sm">
                        {event.type}
                      </div>
                    )}
                  </div>
                  ...
                  {/* Info */}
                  <div className="p-5">
                    <h3 className="text-lg font-bold tracking-tight text-[var(--boldness)] uppercase">
                      {event.title}
                    </h3>
                    ...
                    {event.speaker && (
                      <p className="mt-1 text-sm text-[var(--strategy-red)] italic">
                        {event.speaker}
                      </p>
                    )}

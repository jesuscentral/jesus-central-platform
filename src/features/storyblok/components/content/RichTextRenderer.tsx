...
const resolvers = {
  nodeResolvers: {
    [NODE_UL]: (children: React.ReactNode) => (
      <ul className="my-4 list-disc pl-6">{children}</ul> // no list-inside
    ),
    [NODE_OL]: (children: React.ReactNode) => (
      <ol className="my-4 list-decimal pl-6">{children}</ol> // 1. 2. 3. ...
    ),
  ...
  blokResolvers: {
    ...
    image: (props: SbImage) => {
      return (
        <div className="flex w-full items-center justify-center px-16 py-12">

  ...
  if (payments.length === 0) {
    return (
      <div className="text-center py-12 text-gray-500">
        ...
        <p className="text-sm mt-1">Je hebt nog geen giften gedaan</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {payments.map((payment) => (
        <div
          key={payment.id}
          className="border border-gray-200 rounded-lg p-4 hover:border-brand-orange/50 transition-colors"
        >
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500 mt-1">
                {payment.createdAt &&
                  format(new Date(payment.createdAt), "d MMMM yyyy 'om' HH:mm", {
                    locale: nl,
                  })}
              </p>
              {payment.method && (
            ...
            <div className="text-right">
              ...
              {payment.status === "paid" && payment.paidAt && (
                <p className="text-xs text-green-600 mt-1">
                  Betaald op {format(new Date(payment.paidAt), "d MMM", { locale: nl })}
                </p>
              )}
            </div>
          </div>
